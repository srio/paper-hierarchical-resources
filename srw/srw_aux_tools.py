...
def plot_wfr(wfr,kind='intensity',show=True,xtitle="X",ytitle="Y",title="",aspect='auto'):
	...
	# print(">>>>>>>>>>>>>>>>X: ",x[0],x[-1],x.size,arxx.shape)
	# print(">>>>>>>>>>>>>>>>Y: ",y[0],y[-1],y.size,arxx.shape)
	return ar1,x,y
...
def dump_wfr(f,wfr,prefix="",phase=True,complex_amplitude=True):

    x = numpy.linspace(1e6*wfr.mesh.xStart, 1e6*wfr.mesh.xFin, wfr.mesh.nx)
    y = numpy.linspace(1e6*wfr.mesh.yStart, 1e6*wfr.mesh.yFin, wfr.mesh.ny)


    f[prefix+"_photon_energy"] = wfr.mesh.eStart
    f[prefix+"_x"] = x
    f[prefix+"_y"] = y
    ...
    print(">>>> wavefront added to h5 file")
